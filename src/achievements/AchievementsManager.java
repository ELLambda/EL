...
/**
 * 一个读取和存储成就状况的类 用于和游戏界面交互 用于和读取和存储成就状况的数据类交互
 *
 * @author Andy
 */
...

{
	...
	public static void getAchieveCondition()
	{
		...
		try
		{
			BufferedReader br = new BufferedReader(new FileReader("src/achievements/achieveCondition.txt"));
	...
	public static void setAchieveCondition()
	{
		try
		{
			BufferedWriter bw = new BufferedWriter(new FileWriter("src/achievements/achieveCondition.txt", false));
