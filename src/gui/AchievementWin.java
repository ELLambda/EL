...
public class AchievementWin extends Stage
{
	...
	{ "消灭" + String.valueOf(starElementedNum) + "个该星星，点亮该勋章", "使用了" + String.valueOf(toolUsedNum) + "次该道具，点亮该勋章",
			"总步数超过了" + String.valueOf(stepsUsedNum) + "步，点亮该勋章", "总分数超过了" + String.valueOf(scoreGot) + "分，点亮该勋章",
			"\n点击该勋章可查看积分榜" };
	AnchorPane root;
	...
	public AchievementWin()
	{
		...
		for (int i = 0; i < AchievementsManager.WIDTH; i++)
		{
			...
			achievement.setBackground(new Background(new BackgroundImage(
					new Image("gui/img/achievement/" + String.valueOf(i + 1) + ".png"), BackgroundRepeat.NO_REPEAT,
		...
		for (int i = 0; i < AchievementsManager.WIDTH; i++)
		{
			...
			achievement.setBackground(new Background(new BackgroundImage(
					new Image("gui/img/achievement/" + String.valueOf(i + 6) + ".png"), BackgroundRepeat.NO_REPEAT,
					BackgroundRepeat.NO_REPEAT, BackgroundPosition.CENTER, BackgroundSize.DEFAULT)));
			achievement.setMinSize(100, 120);
			if (i < 3)
			{
		...
		root.setBackground(
				new Background(new BackgroundImage(new Image("gui/img/achievement/bg.jpg"), BackgroundRepeat.NO_REPEAT,
						BackgroundRepeat.NO_REPEAT, BackgroundPosition.CENTER, BackgroundSize.DEFAULT)));
