...
/**
 * 玩家点击四种块分别作为两个加数和和中的两个加数 等式平衡且各项都不为零时游戏结束
 *
 * @author Andy
 */
...
public class GameWinControllor2 extends GameWinControllor
{
	...
	void initialize()
	{
		...
		noticeText.setText(
				adder1.intValue() + "+" + adder2.intValue() + "=" + sumadder1.intValue() + "+" + sumadder2.intValue());
		...
		for (int i = 0; i < BlockManager.length; i++)
		{
			...
			{ // 不变成特效块
				...
				transition.setOnFinished(e ->
				{
					...
					{
						noticeText.setText(adder1.intValue() + "+" + adder2.intValue() + "=" + sumadder1.intValue()
								+ "+" + sumadder2.intValue());
						descend();
	...
	public void checkIsLose()
	{
		if (adder1.intValue() + adder2.intValue() == sumadder1.intValue() + sumadder2.intValue()
				&& adder1.intValue() != 0 && adder2.intValue() != 0 && sumadder1.intValue() != 0
